...
const InvincibleHUD = () => {
  ...
  const label = isActive ? `Invincible ${left.toFixed(1)}s`
    : isCd ? `Cooldown ${Math.ceil(cd)}s` : 'Ready (Space)';
  const pct = isActive ? Math.max(0, Math.min(1, left / 2)) : (isCd ? Math.max(0, Math.min(1, 1 - cd / 30)) : 1);
    ...
    <div className="pointer-events-none absolute bottom-4 right-4">
      <div className="rounded-md bg-black/20 px-3 py-1.5">
        ...
        <div className="w-40 h-1.5 rounded bg-white/10 overflow-hidden">
          <div className="h-full bg-cyan-400" style={{ width: `${(pct * 100).toFixed(0)}%` }} />
        </div>
